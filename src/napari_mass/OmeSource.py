...
class OmeSource:
    ...
    def get_size(self) -> tuple:
        ...
        return np.divide(self.sizes[self.best_level], self.best_factor[:2]).astype(int)
    ...
    def asarray(self, x0: float = 0, y0: float = 0, x1: float = -1, y1: float = -1) -> np.ndarray:
        ...
        if np.mean(factor) != 1:
            h, w = np.round(np.divide(image0.shape[:2], factor)).astype(int)
            image = image_resize_fast(image0, (w, h))
        else:
            image = image0
        if image.shape[:2] != (h0, w0):
